...
public class MyLinkedHashTable<K,V> implements IHashTable<K,V>{
	...
	private List<Node>[] buckets; // list-node type Array, List<Node> for one element 
	private int size;
	...
	public MyLinkedHashTable() {
		...
		for (int i = 0; i < bucketSize; i++) {
			this.buckets[i] = new LinkedList<>(); // if not initialized, null is put in array, so data couldn't be put in.
		}
	}
	...
	private int hash(K key) {
		int hash = 0;
		// we don't know key type one by one, so change the key to String(toString()) and make it CharArray(toCharArray())
